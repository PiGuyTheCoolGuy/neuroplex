...
class Simulation:
    ...
    def __init__(self, config: Config | None = None):
        ...
        self.saved_at = None
        self.event("A new creature is awake. Its brain starts with random connections.")
    ...
    def tick(self):
        ...
        self.brain.reinforce(outcome["reward"])
        self.total_reward += outcome["reward"]
        self.world.sense()  # show the exact retina at the body's new position
        self.ticks += 1
        if outcome["eaten"]:
            self.event(f"Ate {outcome['eaten']} food · reward {outcome['reward']:+.3f}")
        if outcome["died"]:
    ...
    def load(cls, path: Path):
        try:
            with np.load(path, allow_pickle=False) as archive:
                ...
                if metadata["version"] != cls.FORMAT_VERSION:
                    raise ValueError("unsupported checkpoint version")
                sim = cls(Config(**metadata["config"]))
                ...
                for key, value in state.items():
                    if key not in ("paused", "speed", "life", "ticks", "total_reward", "saved_at"):
