...
class Brain:
    ARRAY_NAMES = (
        ...
        "eligibility", "rates", "motor_noise", "motor_rates",
    )
    ...
    def step(self, senses: np.ndarray):
        ...
        # Slow, correlated exploration enters motor neurons as current. It cannot
        # see food coordinates and is not a navigation controller.
        self.motor_noise += (-self.motor_noise * dt / 0.7
                             + self.rng.normal(0, 0.30 * np.sqrt(2 * dt / 0.7), 3))
        drive = np.full(500, 0.92, dtype=np.float32)
        drive[:80] = 0.05 + 2.6 * senses
        drive[400:425] += 0.34 + self.motor_noise[0]
        drive[425:450] -= 0.15
        drive[450:475] += self.motor_noise[1]
        drive[475:500] += self.motor_noise[2]
        ...
        self.threshold[80:] += dt * 0.003 * (self.rates[80:] - 12.0)
    ...
    def advance(self, senses: np.ndarray) -> tuple[float, float]:
        ...
        speed = np.tanh((forward - backward) / 12.0)
        turn = np.tanh((right - left) / 8.0)
